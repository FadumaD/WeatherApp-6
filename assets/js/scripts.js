...
var cityHist = [];
//Saves the current date and storages it 
...
function getWeatherToday() {
	...
	$.ajax({
		...
		//Pictures for the weather used 
		$('.icons').attr('src', `https://openweathermap.org/img/wn/${response.weather[0].icon}@4x.png`);
		// Temperature
		var pEl = $('<p>').text(`Temperature: ${response.main.temp} °F`);
		cardTodayBody.append(pEl);
		//Temp feels like it is
		...
		$.ajax({
			...
			// UV index to match an exposure chart 
			if (uvi >= 0 && uvi <= 2) {
...
function getFiveDayForecast() {
	...
	$.ajax({
		...
		for (let i = 0; i < myWeather.length; i++) {
			...
			var m = moment(`${myWeather[i].date}`).format('DD-MM-YY');
			divElHeader.text(m);
			...
			//Feels Like
			var pElFeel = $('<p>').text(`Feels Like: ${myWeather[i].feels_like} °`);
